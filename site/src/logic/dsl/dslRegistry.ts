...
export const DSL_EVENT_TOOLTIPS: Record<string, string> = {
  ...
  AfterHit: 'Fires right after the nth hit of the move has resolved, e.g. AfterHit(2), or AfterHit(all) for its last hit. With no number it fires after every hit. The hit itself is priced first, so what this applies (buffs, trackers) only affects the hits after it.',
  ...
  OnTick: 'Fires periodically while the mechanic is active: OnTick(3) once every 3 seconds, OnTick(3, 5) at most 5 times. With no number it ticks every second.',
