...
folders under `public/wip-data/`. The folder is gitignored and nothing in it is committed.

- In dev builds, data and image URLs resolve there first (`src/utils/dataSource.ts`). A file that
...
- If a filename isn't obvious, see `DataLoader.mechanicPath` in `src/utils/DataLoader.ts`.

Vite copies `public/` into `dist/`, so a local `npm run build` includes the mirror. Deploys
