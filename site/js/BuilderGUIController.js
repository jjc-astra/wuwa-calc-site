...
const BuilderGUIController = {
    init: async () => {
        ...
        // --- PERSISTENCE: Bind cache execution triggers ---
    ...
    resetCache: async () => {
        if (confirm("Reset current builder cache? All custom nodes and modifications will revert back to pristine database records.")) {
            ...
            // 1. Wipe ONLY the custom data modifications from localStorage (Leave activeChar alone!)
            ...
            // 2. Scrub customized nodes out of the live in-memory runtime databases
            ...
            // 3. Clear DataLoader's cache track list entry to allow re-fetching pristine assets
            ...
            // 4. Force openEditor to re-fetch clean files and repaint your view in-place
            await BuilderGUIController.openEditor(activeChar, BuilderState.activeFolder, BuilderState.activeRarity);
            console.log(`[Cache] Successfully restored pristine database layer for: ${activeChar}`);
        }
    },
    ...
    openEditor: async (itemName, imgFolder = 'Characters', fallbackRarity = 5) => {
        ...
        // --- PERSISTENCE: Set data references for auto-saving scripts ---
        ...
        // --- OPTIMIZED: Trigger the initial render immediately to avoid visual screen popping ---
    ...
    // --- OPTIMIZED: Central MVC Pipeline with Smart Debounce Interception ---
    refreshOutput: (immediate = false) => {
        // Instantiate the cached debouncer closure on first run
    ...
    // The core calculation worker thread
    _executeRefresh: () => {
        ...
        if (typeof LocalCacheManager !== 'undefined') {
            ...
            // --- PERSISTENCE: Dynamically evaluate button activation state mapping rules ---
