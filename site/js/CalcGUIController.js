...
document.addEventListener('DOMContentLoaded', async () => {
    // 1. Await database maps initialization
    if (typeof DataLoader !== 'undefined') {
        await DataLoader.initDatabases();
    }
    
    // 2. Build baseline UI structures and structural card scaffolding
    buildTeamRows();
    generateEchoes();
    RosterUtils.bindRows();
    
    // 3. Bind interactive toolbar configurations and initialize timeline layout
    bindGlobalListeners();
    ...
    // This configures the table headers and injects 1 initial row for standard boots
    bindRotationBuilder(); 

    // 4. NOW intercept and check browser cache to hydrate session records if they exist
    ...
    if (cachedRot) {
        // Overwrites the layout and creates a single trailing placeholder row matching memory array lengths
        await LocalCacheManager.loadPreviousSession(container);
    }
    
    // 5. Finalize tooltip layout calculations paint windows
    TooltipManager.init();
    setTimeout(() => {
...
function bindGlobalListeners() {
    ...
    roster.addEventListener('change', (e) => {
        ...
        if (target.classList.contains('stat-select')) RosterUtils.updateStatRow(target.closest('.stat-row'), true);
        if (target.classList.contains('base-select')) CommonUtils.updatePlaceholderStyle(target);
        
        // --- FIXED: Wire up the Main Stat dropdown listener! ---
        if (target.classList.contains('echo-main-stat-select')) RosterUtils.handleEchoMainStatSelect(target.closest('.echo-card-wrap'), target.value);

        // --- Scrub orphaned Rotation rows when character changes! ---
        if (target.classList.contains('char-select')) {
            RosterUtils.handleCharSelect(target.closest('.char-row'), target.value);
            ...
            document.querySelectorAll('.rotation-row .unit-select').forEach(s => {
                ...
                // If their equipped character is no longer on the team, flush the memory!
                if (!s.value) {
        ...
        if (target.classList.contains('mode-select')) RosterState.updateField(RosterState.domMap.get(target.closest('.char-row')).index, 'mode', target.value);
        if (target.classList.contains('main-set-select')) RosterUtils.handleSetSelect(target.closest('.panel-col'), target.value);
        if (target.classList.contains('layout-select')) RosterUtils.updateEchoMainStats(target.closest('.char-row'), target.value);
        if (target.classList.contains('wep-select')) RosterUtils.handleWeaponSelect(target.closest('.char-row'), target.value);
        if (target.classList.contains('sub-set-select')) RosterUtils.handleSubSetSelect(target.closest('.char-row'), target.value);
        if (target.classList.contains('main-echo-select')) RosterUtils.handleMainEchoSelect(target.closest('.char-row'), target.value);
    });

    roster.addEventListener('click', (e) => {
        const target = e.target;
        
        // Listen for Quick Build Button Click
    ...
    // --- NEW: Bind Enemy Target Configuration ---
    ...
    if (idleContainer) {
        idleContainer.addEventListener('input', (e) => {
            ...
            if (target.classList.contains('enemy-hp-input')) {
                ...
                // Prevent division by zero if the user deletes the text box entirely
...
function bindHeaderPreview() {
    ...
    document.getElementById('team-roster').addEventListener('change', () => requestAnimationFrame(update));
    update();
}
    ...
    fileInput.addEventListener('change', (e) => {
        ...
        reader.onload = (ev) => { 
            try { 
                ...
                console.error('[CalcGUIController] Error parsing or applying team import JSON:', err);
...
// Sub-Binder: Checkbox / Selection Logic
function _bindRotationSelection(container, state) {
    container.addEventListener('click', (e) => {
        // --- FIXED: Target the label wrapper, not the input! ---
        const wrap = e.target.closest('.check-wrap');
        if (!wrap) return;
        
        // Stop the browser's native label behavior so we don't get ghost clicks
        e.preventDefault(); 
        ...
        // Calculate the intended state manually since we prevented the default action
        ...
        if (isShift && state.lastSelectedIndex !== null) {
            ...
            // Exclusive Selection (Uncheck all others)
            rows.forEach((r, i) => {
                if (i !== currentIndex) {
                    const rCb = r.querySelector('.row-select-check');
            ...
            // Apply intended state to the clicked row
...
// Sub-Binder: CRUD Actions (Add, Delete, Copy, Paste)
function _bindRotationCommands(container, state) {
    const performDelete = () => {
        ...
        if (toDelete.length > 0) {
            // --- FIXED: Do not call removeRow here manually! Let the command handle it. ---
    ...
    const performInsert = (direction) => { 
        ...
        // --- OPTIMIZED: Calculate target strictly from memory array ---
    ...
    const performCopy = () => {
        ...
        // --- NEW: Deselect everything so the user knows the copy was successful ---
        ...
        // The copy button will disable itself, and the paste button will light up!
        RotationUtils.updateActionButtons(container, state.clipboard);
    };
    ...
    const performPaste = () => {
        ...
        // Grab all selected rows except the empty placeholder
        ...
        // Deselect everything after pasting
        container.querySelectorAll('.rotation-row.selected').forEach(r => { 
            r.classList.remove('selected'); 
            const cb = r.querySelector('.row-select-check');
            if (cb) cb.checked = false; 
    ...
    // --- FIXED: Bind the new split buttons! ---
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Delete' || e.key === 'Backspace') {
            ...
            if (active.tagName !== 'TEXTAREA' && !active.isContentEditable && !(active.tagName === 'INPUT' && !['checkbox','radio','button'].includes(active.type))) {
                e.preventDefault(); performDelete(); 
            }
        }
...
// Sub-Binder: Export/Import/Calculate
function _bindRotationExternal(container) {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            ...
            // --- NEW: Inject settings into export ---
            const exportObject = { 
            ...
            a.download = RosterUtils.generateFilename("Rotation"); a.click();
        });
    }
        ...
        fileInput.addEventListener('change', (e) => {
            ...
            reader.onload = async (ev) => {
                try {
                    ...
                    // --- NEW: Apply imported settings to checkboxes and state ---
                    if (rawData.settings) {
                        RotationState.startEnergy = rawData.settings.startEnergy !== false; // default to true
                        RotationState.startConcerto = rawData.settings.startConcerto === true; // default to false
                    ...
                    // 1. AWAIT the team import first so all necessary JSONs are fetched into MECHANICS_DB
                    if (rawData.team) {
                        await RosterUtils.applyTeamImport(rawData.team); 
                    }
                    
                    // 2. NOW it is perfectly safe to build the rotation!
                    if (rotData) {
                        ...
                        RotationUtils.runSimulation(); // Ensure the math runs after import
                    }
                } catch (err) { 
                    console.error('[CalcGUIController] Error parsing or applying rotation import JSON:', err);
                    alert("Error loading rotation. Check console for details."); 
                }
            };
            reader.readAsText(file); fileInput.value = '';
        });
    ...
    if (calcBtn) {
        ...
        warnLabel.id = 'rot-calc-warning'; warnLabel.className = 'calc-warning-msg';
...
// Sub-Binder: DOM Interaction Logic
function _bindRotationEvents(container, headerRow, state) {
    ...
    container.addEventListener('dragstart', (e) => {
        ...
        if (!row || row === container.lastElementChild) { // Prevent dragging the empty placeholder
            e.preventDefault();
            return;
        }

        // If the dragged row is part of a selection, drag ALL selected rows. Otherwise, just the dragged row.
        if (row.classList.contains('selected')) {
            draggedRows = Array.from(container.querySelectorAll('.rotation-row.selected')).filter(r => r !== container.lastElementChild);
        } else {
            draggedRows = [row];
        }
    ...
    container.addEventListener('dragover', (e) => {
        ...
        // Determine if we are dropping ABOVE or BELOW the target row
        ...
        if (targetRow !== container.lastElementChild) {
            ...
            targetRow.classList.add('drag-over-top'); // Always drop above the empty placeholder
        }
    });
    ...
    container.addEventListener('drop', (e) => {
        ...
        // Adjust target index if dropping below the row
        if (isBelow && targetRow !== container.lastElementChild) {
            targetIndex++;
        }

        // Execute move via Command History
