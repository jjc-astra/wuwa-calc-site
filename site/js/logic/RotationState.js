...
const RotationState = {
    ...
    _aggregateBuffTotals: function(stateContext, executingUnit, hitModifiers) {
        const buffTotals = {
        ...
        Object.entries(stateContext.activeBuffs).forEach(([bKey, buff]) => {
            ...
            if (buff.applyTo && Array.isArray(buff.applyTo) && buff.applyTo.length > 0) {
                const isValid = buff.applyTo.some(req => hitModifiers.includes(String(req).toLowerCase()));
                if (!isValid) return; // Skip applying this buff to this specific hit!
            }

            let hasApplicableStat = false;
            if (buff.stat) {
                const s = buff.stat.toLowerCase();
                
                if (buff.name === "Lumi_Laser Boost") {
                    console.log(`[DEBUG 3: AGGREGATE] Processing S5 Buff. Stat: '${buff.stat}' (Parsed lower: '${s}'), Value: ${buff.value}`);
                }

                const requiresSpecificMod = ["basic", "heavy", "skill", "liberation", "lib", "glacio", "fusion", "electro", "aero", "spectro", "havoc", "physical"].find(word => s.includes(word));
                if (requiresSpecificMod) {
                    const requiredMod = requiresSpecificMod === "lib" ? "liberation" : requiresSpecificMod;
                    if (!hitModifiers.includes(requiredMod)) return; 
                }
                ...
                const isPct = String(evaluatedVal || "0").includes('%');
                let numVal = parseFloat(evaluatedVal || "0") || 0;
                if (isPct) numVal = numVal / 100;
                
                const val = numVal * (buff.stacks || 1);
                hasApplicableStat = true; 
                const clean = s.replace(/[^a-z]/g, '');

                if (isSelf) {
                    if (clean === "atk" || clean === "percentatk" || clean === "flatatk") { if (isPct) buffTotals.percentAtk += val; else buffTotals.flatAtk += val; }
                    else if (clean === "hp" || clean === "percenthp" || clean === "flathp") { if (isPct) buffTotals.percentHP += val; else buffTotals.flatHP += val; }
                    ...
                    else if (clean.includes("additivemult")) buffTotals.additiveMult += val;
                }
                if (isEnemy) {
                    if (clean.includes("dmgtaken") || clean.includes("taken")) buffTotals.dmgTaken += val;
                    else if (clean.includes("reduceres") || clean.includes("resshred")) buffTotals.reduceRes += val;
                    else if (clean.includes("reducedef")) buffTotals.reduceDef += val;
                }
            }
            if (hasApplicableStat) appliedBuffs[bKey] = buff; 
        });
        return { buffTotals, appliedBuffs };
    },

    _calculateDamageInstance: function(hitConfig, stateContext) {
        ...
        const baseStats = (typeof StatCalculator !== 'undefined') ? StatCalculator.calculateFinalStats(executingUnit, []) : {};
        ...
        if (titleStr.includes("Laser Beam")) {
            ...
            console.log(`[DEBUG 2: LASER] Aggregated Multiplicative Mult for this hit:`, buffTotals.multiplicativeMult);
        }
